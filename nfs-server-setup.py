...
if dry_run:
    print('/media/ ' + Ip + '(' + st + ',sync,crossmnt,no_root_squash,no_subtree_check)')
else:
    print('/media/ ' + Ip + '(' +
          st + ',sync,crossmnt,no_root_squash,no_subtree_check)', file=open("/etc/exports", "a"))
...
while True:
    if yes_or_no("Do you wish to setup any additional shares " +
        ...
        st = ro_or_rw("Should the share be read/write or read only? ")
        if dry_run:
            print(share, Ip + '(' + st + ',sync,crossmnt,no_root_squash,no_subtree_check)')
        else:
            print(share, Ip + '(' + st + ',sync,crossmnt,no_root_squash,no_subtree_check)',
